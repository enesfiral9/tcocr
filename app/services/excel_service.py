...
HEADERS = ["Sıra No", "T.C. Kimlik No", "Ad", "Soyad", "Doğum Tarihi", "Seri No", "OCR Güven Oranı", "Kontrol Durumu"]
...
def create_excel(records: list[ExportRecord]) -> BytesIO:
    ...
    for index, record in enumerate(records, 1):
        sheet.append([index, record.tc_no, record.name, record.surname, record.birth_date, record.serial_no,
                      record.confidence, "Kontrol Gerekli" if record.requires_review else "Başarılı"])
        sheet.cell(index + 1, 7).number_format = "0%"
    sheet.freeze_panes = "A2"
    sheet.auto_filter.ref = f"A1:H{max(1, sheet.max_row)}"
    for index, width in enumerate([10, 18, 22, 22, 18, 18, 20, 22], 1):
